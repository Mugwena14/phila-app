...
export default function ProfileScreen() {
  ...
  const load = async (): Promise<void> => {
    try {
      ...
      // Show health onboarding if profile is empty and never shown before
      const shown = await AsyncStorage.getItem('health_onboarding_shown')
      if (!shown && !data?.blood_type && !data?.height_cm) {
        setShowHealthOnboarding(true)
        await AsyncStorage.setItem('health_onboarding_shown', 'true')
      }
    } catch {
      // silent fail
    }
  ...
  return (
    <>
      <ScrollView style={{ flex: 1, backgroundColor: colors.bgBase }} showsVerticalScrollIndicator={false}>
        ...
        <View style={{ marginHorizontal: spacing.lg, marginBottom: spacing.lg }}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: spacing.md }}>
            <Text style={{ fontFamily: 'Syne_700Bold', fontSize: 15, color: colors.text }}>Health info</Text>
            {/* Edit health profile shortcut */}
            <TouchableOpacity onPress={() => setShowHealthOnboarding(true)}>
              <Text style={{ fontFamily: 'DMSans_500Medium', fontSize: 13, color: colors.primary }}>Edit</Text>
            </TouchableOpacity>
          </View>
          <View style={{ backgroundColor: colors.bgSurface, borderRadius: radius.xl, borderWidth: 1, borderColor: colors.border, overflow: 'hidden' }}>
            {HEALTH_STATS.map((stat, i) => (
              <View
                key={stat.label}
                style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: spacing.lg, paddingVertical: 14, borderBottomWidth: i < HEALTH_STATS.length - 1 ? 1 : 0, borderBottomColor: colors.border, gap: spacing.md }}
              >
                ...
                <Text style={{ fontFamily: 'Syne_700Bold', fontSize: 14, color: stat.value === '—' ? colors.textFaint : colors.primary }}>
                  {stat.value}{' '}
